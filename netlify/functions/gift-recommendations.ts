...
interface GiftRecommendationRequest {
  ...
  preferences: {
    ...
    prioritizeFreeShipping: boolean;
...
export const handler: Handler = async (event, context) => {
  ...
  try {
    // Parse request body
    const requestData: GiftRecommendationRequest = JSON.parse(event.body || '{}');
...
[
  {
    ...
    try {
      ...
      recommendations = parsedRecommendations.map((rec: any, index: number) => ({
