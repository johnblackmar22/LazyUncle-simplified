...
const handler = async (event) => {
  ...
  try {
    ...
    const prompt = `You are an expert gift recommendation assistant.\n\nIMPORTANT:\n- Carefully consider all recipient information, interests, and special instructions provided. Recommendations should be personal and relevant.\n- Respond only with valid JSON in the format below—no explanations, no extra text, no Markdown.\n- Recommend exactly 3 real, popular, and currently in-stock gifts from Amazon.com for the recipient and occasion below.\n- Use the exact product name as listed on Amazon.\n- For each gift, include:\n  * name\n  * description\n  * price (USD)\n  * a direct Amazon purchase URL (make sure it's a real link, or state \"URL not available\" if unsure)\n  * estimated shipping cost (if not free)\n  * whether gift wrapping is available (true/false)\n  * why this gift is a good fit for the recipient and occasion, specifically referencing the recipient's interests or instructions.\n- Gifts must be as close as possible to the budget (never exceeding it); at least one gift must be within $1 of the budget. Do not suggest gifts that are significantly under budget unless absolutely necessary.\n- Do not suggest generic items or categories—be specific.\n- If the recipient is a child, all gifts must be age-appropriate and safe.\n- Only use information provided—do not guess missing details.\n- Do not include explanations, headers, or comments—just valid JSON.\n${previousGiftNames && previousGiftNames.length > 0 ? `\nDo not recommend any of the following gifts: ${previousGiftNames.join(', ')}.` : ''}\n\nRecipient:\n${JSON.stringify(recipient, null, 2)}\nOccasion:\n${JSON.stringify({ ...occasion, budget: budget.total, instructions: instructions || '' }, null, 2)}\n\nJSON response format:\n{\n  \"recommendations\": [\n    {\n      \"name\": \"...\",\n      \"description\": \"...\",\n      \"price\": ...,\n      \"purchaseUrl\": \"...\",\n      \"shippingCost\": ...,\n      \"giftWrappingAvailable\": true/false,\n      \"reasoning\": \"... (reference recipient info and instructions)\"\n    },\n    ...\n  ]\n}`;
